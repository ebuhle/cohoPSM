...
plot(d$Z_mean[1:51],d$p_psm_mean[1:51], pch=d$psmshape, col="white", yaxt='n',xaxt='n',cex.lab=1.5,cex.axis=1.5,cex=1.52, 
...
plot(d$Z_mean[1:51],d$p_psm_mean[1:51], pch=d$psmshape, col="gray", cex.lab=1.2,cex.axis=1.2,cex=1.52, 
...
dxy<-cbind(d$site.x,dxy,d$Coho_Presence_m,score)
...
colnames(dxy)[1:4]<-c("site","Z","benefit.stan","benefit")
...
dxy<-cbind(d$site.x,dxy,d$nsp_pres,score)
dxy<-dxy[order(dxy$score),]
myPalette <- colorRampPalette(brewer.pal(9, "YlGnBu")) #### Gives us a heat map look
cols = myPalette(length(score))
dxy<- data.frame(cbind(dxy,cols))
...
mtext(side=1,"high",line=4,adj=1,cex=0.8)
mtext(side=1,"low",line=4,adj=0,cex=0.8)
dxy<-dxy[-which(is.na(dxy$site)),]
score_salmonnpp<-dxy
score_cohopres_m<-score_cohopres_m[-which(is.na(score_cohopres_m$site)),]
colnames(score_salmonnpp)[3:6]<-c("numspp.stan","numspp","score.numsp","cols.numsp")
scores<-full_join(score_cohopres_m,score_salmonnpp)
scores<-scores[,-6]
scores<-scores[,-9]
