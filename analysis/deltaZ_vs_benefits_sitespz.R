...
colnames(score_cohosppres_m)[3:6]<-c("coho.pres.stan","coho.pres.m","score.coho.pres","cols.cohosppres")
colnames(score_nsp)[3:6]<-c("nsp.stan","nsp","score.nsp","cols.nsp")

scores<-full_join(score_cohosppres_m,score_chinsppres_m)
scores<-left_join(scores,score_nsp)



#fix it up for Blake: remove "NAs" and replace with -9999 and remove colums that we don't care about
scores_forblake<-scores[order(scores$ID),]
