...
zplotfx <- function(psm_thresh,attribut){
  ...
  plot(dxy$delta_Z,dxy$benefit, cex=3,cex.lab=1.2,cex.axis=1.2,xlab="",ylab="", ylim = c(0, max(dxy$benefit)), type="p", pch=d$psmshape, col=dxy$cols, bty="l",mgp=c(1.5,.5,0), tck = -0.01, cex.axis = 1.5, cex.lab = 1.5, yaxs = "i", xaxs = "r", las = 1)
  ...
  plot(dxy$delta_Z,dxy$priority, cex=3,cex.lab=1.5,cex.axis=1.5,ylim = c(dim(dxy)[1],1),xlab="", ylab= "", type="p", pch=d$psmshape, col=dxy$cols, bty="l",mgp=c(1.5,.5,0), tck =-0.01,cex.axis = 1.5, cex.lab = 1.5, yaxs = "i", xaxs = "r", las = 1)
...
text(min(d$Z_mean, na.rm=TRUE)+1,input$psm_thresh+.02,label="Pre-spawn mortality threshold", cex=1.2)
